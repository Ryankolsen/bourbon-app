...
export function BourbonCard({
  ...
  return (
    ...
    >
      {/* Bourbon name */}
      <Text className="text-bourbon-100 font-bold text-base">{name}</Text>

      {/* Row 1: distillery and type */}
      {hasRow1 && (
        <View className="flex-row gap-2 mt-0.5">
          {distillery ? (
            <Text className="text-bourbon-400 text-sm">{distillery}</Text>
          ) : null}
          {type ? (
            <Text className="text-bourbon-400 text-sm">{type}</Text>
          ) : null}
        </View>
      )}
      ...
      {hasRow2 && (
        <View className="flex-row gap-4 mt-1">
          {proof != null ? (
            <Text className="text-bourbon-400 text-xs">{proof} proof</Text>
          ) : null}
          {age != null ? (
            <Text className="text-bourbon-400 text-xs">{age} yr</Text>
          ) : null}
        </View>
      )}
      ...
      <View className="flex-row justify-between items-center mt-2">
        <View className="flex-row items-center gap-1">
          <Text className="text-bourbon-500 text-xs">You</Text>
          <StarRating value={personalRating ?? null} variant="personal" size="sm" />
        </View>
        <View className="flex-row items-center gap-1">
          <Text className="text-bourbon-500 text-xs">Community</Text>
